"""Headless arithmetic engine for MiniCalc Standard Mode."""
...
class CalculatorEngine:
    ...
    def replace_current_input(self, text: str) -> str:
        """Replace the active input/display with a Decimal-compatible value.

        Used by Standard Mode memory recall. It preserves pending operators when
        entering/replacing the second operand, starts a new calculation after a
        displayed result, and is a no-op while the engine is in an error state.
        """
        if self.state == self.ERROR:
            return self.display
        try:
